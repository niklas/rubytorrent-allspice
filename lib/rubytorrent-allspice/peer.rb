...
require 'socket'
require 'thread'
require "rubytorrent-allspice/message"
...
module ArrayToBitstring
  ...
  #       bit = 7
  #     end
  #     ret[ret.size - 1] |= (1 << bit) if b
  #     bit -= 1
  #   end
  #   ret
  # end
  
  # Equivalent (I hope) version that is Ruby 1.9.2 compatible
  def to_bitstring
    ret = "\0"
    bit = 7
    map do |b|
      ...
      # ret[ret.size - 1] |= (1 << bit) if b
      ret[ret.size - 1] = [ret[ret.size - 1].unpack("C*").first + (1 << bit)].pack("C*") if b
      bit -= 1
    end
    ret
  end
...
#        rt_debug e.backtrace.join("\n")
    ...
    # puts "@package.pieces.inspect #{@package.pieces.inspect}"
...
#        puts "[][] #{self}: #{b} is #{b.requested? ? 'requested' : 'NOT requested'} and has time_elapsed of #{b.requested? ? b.time_elapsed.round : 'n/a'}s"
        if b.requested? && (b.time_elapsed > REQUEST_TIMEOUT)
          ...
          # puts "#{self}: for block #{b}, time elapsed since request is #{b.time_elapsed} > #{REQUEST_TIMEOUT}, assuming peer forgot about it"
...
#    rt_debug "sending blocks. choking? #@choking, choked? #@peer_choking, ul rate #{ulrate}b/s, limit #@ulmeterlim" unless @peer_want_blocks.empty?
    ...
    # puts "REQD BYTES: #{reqd_bytes}"
    # puts "SENT BYTES: #{sent_bytes}"
...
#      rt_debug "output: sending message #{obj}" + (obj.id == :request ? " (request queue size #{@want_blocks.length})" : "")
      # puts "output: sending message #{obj}" + (obj.id == :request ? " (request queue size #{@want_blocks.length})" : "")
...
#      rt_debug "* hey, a keepalive!"
    ...
    full = recv_bytes(1)
    id = full[0].unpack("C*").first
    
    # puts "ID #{id.class.name}: '#{id.inspect}' '#{full.inspect}'"    
    # puts Message::WIRE_IDS[id].inspect
    ...
    if Message::WIRE_IDS[id] == :piece # add a block
      ...
      # puts "WIRE_IDS: #{Message::WIRE_IDS[id].inspect}"
      # puts "len '#{len.inspect}'"
...
#      rt_debug "#{self}: peer choking (was #{@peer_choking})"
      # puts "#{self}: peer choking (was #{@peer_choking})"
      @peer_choking = true
      @want_blocks_m.synchronize do
        @controller.forget_blocks @want_blocks
        @want_blocks.clear
...
#      rt_debug "#{self}: peer not choking (was #{@peer_choking})"
      # puts "#{self}: peer not choking (was #{@peer_choking})"
      @peer_choking = false

    when :interested
#      rt_debug "peer interested (was #{@peer_interested})"
      # puts "peer interested (was #{@peer_interested})"
      @peer_interested = true

    when :uninterested
#      rt_debug "peer not interested (was #{@peer_interested})"
      # puts "peer not interested (was #{@peer_interested})"
      @peer_interested = false
      
    when :have
#      rt_debug "peer has piece #{m.index}"
      # puts "peer has piece #{m.index}"
...
#      rt_debug "peer reports bitfield #{m.bitfield.inspect}"
      ...
      # puts "Expected Pieces: #{expected_pieces}"
      # puts "barray.length  : #{barray.length}"
      puts "invalid length in bitfield message (package has #{@package.num_pieces} pieces; bitfield should be size #{expected_pieces} but is #{barray.length} pieces)" unless barray.length == expected_pieces
      ...
      # puts "Peer Pieces is: #{@peer_pieces.inspect}"
      @peer_virgin = false
  ...
  def get_want_blocks
    ...
    @controller.claim_blocks do |b|
      ...
      if @peer_pieces[b.pindex] && !@want_blocks.member?(b)
        ...
        # puts "! #{self}: starting new piece #{@package.pieces[b.pindex]}" unless @package.pieces[b.pindex].started?
