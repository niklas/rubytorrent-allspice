...
torrent = Dir.glob(File.join(tmp_folder, "*.torrent")).first
if torrent == nil
...
#      @window.erase
    ...
    [
      ...
      "",
      "",
      "Press 'Q' to Quit.  Your download will continue where it left off the next time you run this program.",
    ].each do |s|
      break if line > @rows
#      @window.mvaddnstr(line, 2, s + (" " * @cols), @cols - 4)
      @window.setpos(line, 2)
      @window.addstr(s + (" " * @cols))
      line += 1
    end
...
#    @window.mvaddstr(5, @cols - 11, sprintf("] %.2f%%  ", (@completed.to_f / @total) * 100.0))
	...
	@window.setpos(5, @cols - 11)
	@window.addstr(sprintf("] %.2f%%  ", (@completed.to_f / @total) * 100.0))
    
    
#    @window.mvaddnstr(7, 31, "|" + ("#" * (@dlrate / 1024)) + (" " * @cols), @cols - 31 - 2)
#	@window.setpos(7, 31)
#	@window.addstr("|" + ("#" * (@dlrate / 1024)) + (" " * @cols))
	
#    @window.mvaddnstr(8, 31, "|" + ('#' * (@ulrate / 1024)) + (" " * @cols), @cols - 31 - 2)
#	@window.setpos(8, 31)
#	@window.addstr("|" + ('#' * (@ulrate / 1024)) + (" " * @cols))
...
#  Curses.stdscr.nodelay(true)
  ...
  display.draw; Curses.refresh
  bt = RubyTorrent::BitTorrent.new(mi, package, :http_proxy => proxy, :port => port, :dlratelim => dlratelim, :ulratelim => ulratelim) # Added Ruby 1.9.2
